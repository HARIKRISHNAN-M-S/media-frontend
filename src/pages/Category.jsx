...
import { addCategory, getallCategory, deleteCategory, getvideo, updateCategory } from '../services/Allapi';
...
function Category() {
  ...
  const handleDeletCategory = async (e, id) => {
    e.preventDefault()
    console.log(id);
  ...
  const dropped=async(e,categoryId)=>{
    ...
    console.log("sourcecardid is",sourcecardId);

    // logic to impliment adding card in the given catrgory
        ...
        allCategory?.map(item => (
          <div droppable onDrop={e=>dropped(e,item?.id)}  onDragOver={e => dragover(e)} >
             <div className='d-flex justify-content-between border rounded mt-2 p-3'>
              ...
              <span onClick={e => handleDeletCategory(e, item?.id)}     ><Trash2 color="red"></Trash2></span>
